 ...
 protected:
  ConcatLayerTest()
      : blob_bottom_0_(new Blob<Dtype>(2, 3, 6, 5)),
        ...
        blob_bottom_3_(new Blob<Dtype>(2, 3, 6, 5)),
  ...
  virtual void SetUp() {
    ...
    blob_bottom_vec_2_.push_back(blob_bottom_0_);
    blob_bottom_vec_2_.push_back(blob_bottom_3_);
    blob_top_vec_.push_back(blob_top_);
  }

  virtual ~ConcatLayerTest() {
    delete blob_bottom_0_; delete blob_bottom_1_;
    delete blob_bottom_2_; delete blob_bottom_3_; delete blob_top_;
  }
  ...
  vector<Blob<Dtype>*> blob_bottom_vec_0_, blob_bottom_vec_1_,blob_bottom_vec_2_;
...
TYPED_TEST(ConcatLayerTest, TestForwardTrivialInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_interleaved(true);
  ConcatLayer<Dtype> layer(layer_param);
  this->blob_bottom_vec_0_.resize(1);
  layer.SetUp(this->blob_bottom_vec_0_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_0_, this->blob_top_vec_);
...
TYPED_TEST(ConcatLayerTest, TestForwardNumInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_axis(0);
  layer_param.mutable_concat_param()->set_interleaved(true);
  ...
  for (int n = 0; n < this->blob_bottom_vec_2_[0]->num(); ++n) {
    for (int c = 0; c < this->blob_top_->channels(); ++c) {
      for (int h = 0; h < this->blob_top_->height(); ++h) {
        for (int w = 0; w < this->blob_top_->width(); ++w) {
          EXPECT_EQ(this->blob_top_->data_at(n*2, c, h, w),
              this->blob_bottom_vec_2_[0]->data_at(n, c, h, w));
        }
      }
    }
  }
  for (int n = 0; n < this->blob_bottom_vec_2_[1]->num(); ++n) {
    for (int c = 0; c < this->blob_top_->channels(); ++c) {
      for (int h = 0; h < this->blob_top_->height(); ++h) {
        for (int w = 0; w < this->blob_top_->width(); ++w) {
          EXPECT_EQ(this->blob_top_->data_at(n*2+1, c, h, w),
              this->blob_bottom_vec_2_[1]->data_at(n, c, h, w));
        }
      }
    }
  }
...
TYPED_TEST(ConcatLayerTest, TestForwardChannelsInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_interleaved(true);
  ConcatLayer<Dtype> layer(layer_param);
  ...
  layer.Forward(this->blob_bottom_vec_2_, this->blob_top_vec_);
  for (int n = 0; n < this->blob_top_->num(); ++n) {
    for (int c = 0; c < this->blob_bottom_vec_2_[0]->channels(); ++c) {
      for (int h = 0; h < this->blob_top_->height(); ++h) {
        for (int w = 0; w < this->blob_top_->width(); ++w) {
          EXPECT_EQ(this->blob_top_->data_at(n, c*2, h, w),
              this->blob_bottom_vec_2_[0]->data_at(n, c, h, w));
        }
      }
    }
    for (int c = 0; c < this->blob_bottom_vec_2_[1]->channels(); ++c) {
      for (int h = 0; h < this->blob_top_->height(); ++h) {
        for (int w = 0; w < this->blob_top_->width(); ++w) {
          EXPECT_EQ(this->blob_top_->data_at(n, c*2+1, h, w),
              this->blob_bottom_vec_2_[1]->data_at(n, c, h, w));
        }
      }
    }
...
TYPED_TEST(ConcatLayerTest, TestGradientTrivialInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_interleaved(true);
  ConcatLayer<Dtype> layer(layer_param);
...
TYPED_TEST(ConcatLayerTest, TestGradientNumInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_axis(0);
  layer_param.mutable_concat_param()->set_interleaved(true);
...
TYPED_TEST(ConcatLayerTest, TestGradientChannelsInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_interleaved(true);
  ConcatLayer<Dtype> layer(layer_param);
...
TYPED_TEST(ConcatLayerTest, TestGradientChannelsBottomOneOnlyInterleaved) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_concat_param()->set_interleaved(true);
  ConcatLayer<Dtype> layer(layer_param);

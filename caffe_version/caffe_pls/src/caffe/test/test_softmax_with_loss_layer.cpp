...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradientWeighted) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.add_loss_weight(3);
  layer_param.mutable_loss_param()->add_beta(.1);
  layer_param.mutable_loss_param()->add_beta(.2);
  layer_param.mutable_loss_param()->add_beta(.15);
  layer_param.mutable_loss_param()->add_beta(.25);
  layer_param.mutable_loss_param()->add_beta(.3);
  SoftmaxWithLossLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-2, 1701);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_, 0);
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestForwardIgnoreLabelWeighted) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_loss_param()->set_normalize(false);
  layer_param.mutable_loss_param()->add_beta(.1);
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradientIgnoreLabelWeighted) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  // labels are in {0, ..., 4}, so we'll ignore about a fifth of them
  layer_param.mutable_loss_param()->set_ignore_label(0);
...
TYPED_TEST(SoftmaxWithLossLayerTest, TestGradientUnnormalizedWeighted) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_loss_param()->set_normalize(false);
  layer_param.mutable_loss_param()->add_beta(.1);

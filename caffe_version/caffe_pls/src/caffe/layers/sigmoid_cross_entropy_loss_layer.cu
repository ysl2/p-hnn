...
template <typename Dtype>
__global__ void SigmoidCrossEntropyLossForwardGPUBeta(const int nthreads,
          const Dtype* input_data, const Dtype* target, Dtype* loss,
          const bool has_ignore_label_, const int ignore_label_, const float beta,
          Dtype* counts) {
  CUDA_KERNEL_LOOP(i, nthreads) {
    const int target_value = static_cast<int>(target[i]);
    if (has_ignore_label_ && target_value == ignore_label_) {
      ...
      loss[i]= ((target_value)*(1-2*beta)+beta)*(input_data[i] * (target_value - (input_data[i] >= 0)) -
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
    SigmoidCrossEntropyLossForwardGPUBeta<Dtype><<<CAFFE_GET_BLOCKS(count),
        CAFFE_CUDA_NUM_THREADS>>>(count, input_data, target, loss_data,
        has_ignore_label_, ignore_label_, beta_,count_data);
  }
  else{
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    if (is_beta_){
      ...
      caffe_gpu_mul_add(count,loss_weight*(1-2*beta_),target,loss_weight*beta_,bottom_diff,bottom_diff);

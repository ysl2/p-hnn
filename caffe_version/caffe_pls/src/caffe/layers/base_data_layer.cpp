...
BasePrefetchingLabelmapDataLayer<Dtype>::BasePrefetchingLabelmapDataLayer(
  ...
  for (int i = 0; i < prefetch_.size(); ++i) {
    prefetch_[i].reset(new LabelmapBatch<Dtype>());
    prefetch_free_.push(prefetch_[i].get());
  }
...
void BasePrefetchingLabelmapDataLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  labelmap_number_  = top.size()-1;
  CHECK((labelmap_number_>0)) << "Must specify at least one top output for labelmaps.";


  BaseDataLayer<Dtype>::LayerSetUp(bottom, top);
  std::cerr << "Finished based setup" << std::endl;
  ...
  for (int i = 0; i < prefetch_.size(); ++i) {
    ...
    for (int j=0;j<labelmap_number_;++j){
      ...
      prefetch_[i]->labelmaps_[j]->mutable_cpu_data();
    }
  ...
  if (Caffe::mode() == Caffe::GPU) {
    for (int i = 0; i < prefetch_.size(); ++i) {
      std::cerr<< "Before datat check" << std::endl;
      prefetch_[i]->data_.mutable_gpu_data();
      std::cerr<< "Passed datat check" << std::endl;
      for (int j=0;j<labelmap_number_;++j){

        prefetch_[i]->labelmaps_[j]->mutable_gpu_data();
      }
...
void BasePrefetchingLabelmapDataLayer<Dtype>::Forward_cpu(
  ...
  for (int j=0;j<labelmap_number_;++j){
    ...
    top[j+1]->set_cpu_data(prefetch_current_->labelmaps_[j]->mutable_cpu_data());
  }
